...
app.get("/states/:id", (req, res) => {
    ...
    const state = states.find(stat => stat.id === id);
    if (!state) {
...
app.get("/state/highest-gdp", (req, res) => {
    const state = states.reduce((max, stat) => (max.gdp > stat.gdp) ? max : stat, states[0]);
    if (!state) {
        return res.status(404).json({ "error": "State not found" });
    }
    res.status(200).json(state);
...
app.post("/states", (req, res) => {
    ...
    try {
        ...
        res.status(404).json({ "error": "Something wants wrong" });
    }
});
...
app.put("/states/:id", (req, res) => {
    const id = Number(req.params.id);
    const i = states.findIndex(state => state.id === id);
    if (i === -1) {
...
app.delete("/states/name/:stateName", (req, res) => {
    ...
    const i = states.findIndex(state => state.name.toLowerCase() === name);
    if (i === -1) {
        return res.status(404).json({ "error": "Invalid id" });
    }
    const state = states.splice(i, 1);
